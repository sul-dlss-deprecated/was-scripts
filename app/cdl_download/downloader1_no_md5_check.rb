...
class Sdsc
  def initialize(user, pwd)
  	@fog = Fog::Storage.new(:provider => 'OpenStack',
               ...
               :connection_options => {chunk_size: 102_400, connect_timeout: 600, read_timeout: 600, write_timeout:600})
  	@fog.change_account('AUTH_wasprod')
  end
  ...
  def write_container(fog_container)
    ...
    Sdsc.retry_sdsc do
      ...
      file_list.each do |fog_file|
        write_file(fog_file, dirname, fog_container)
  ...
  def write_file(fog_file, container_dir, fog_container)
    Sdsc.retry_sdsc do
      ...
      File.open(fn, 'w') do |local_file|
        ...
        #throw "#{fog_file[:key]} Bad md5 error" if fog_file[:etag] != md5
      #end
    end
  end
